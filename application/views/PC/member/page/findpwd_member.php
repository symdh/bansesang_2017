	<div id = "member_L_main">
		<div id = "border_L_wrapper" >
			...
			<div id = "member_L_wrapper" >
				...
				<form name="findpwd" method="post" action="/member/tryfindpwd " onsubmit = 'find_form_check(); return false;'>
				<input type='hidden' name='<?=$this->security->get_csrf_token_name()?>' value='<?=$this->security->get_csrf_hash()?>' />
					<table>
						...
						<tr>
							<td colspan="2">
									<span style = "float:right;" class = 'C_height_full'>
										아이디 : <input  type='text' name='user_email_id'  placeholder="아이디를 입력하세요." size='25'  onkeydown = "if(window.event.keyCode == 9){ $('input[name=user_name]').focus(); return false;}" >
									</span>
						...
						<tr>
							<td colspan="2">
								<span style = "float:right;" class = 'C_height_full' >
									가입시 입력한 이름 : <input  type='text' name='user_name' placeholder="가입시 입력했던  가장 친한 친구이름" size='25' onkeydown = "if(window.event.keyCode == 9){ $('input[name=user_phone]').focus(); return false;}" >
								</span>
							</td>
						</tr>

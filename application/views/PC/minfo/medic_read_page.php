...
/////////////////////클라이언트 기본 변수 설정///////////////////////
   ...
   //comment_id의 가장 마지막 값을 구하면서 배열로 따로 저장
	...
	echo "lately_read_id = {$data_w[$data_w_read[0]['divide_type']][0]['read_id']};"; 
		//글쓴이
	echo "read_writed_member = '{$data_w[$data_w_read[0]['divide_type']][0]['writed_member']}';";
		//현재 페이지 확인
...
<?php include "./static/include/view/board/sub_list.php"; ?>
...
function w_modify_form ($data_w_read, $check_loggin, $function_name) {
	...
	echo "<form action = '/minfo/{$function_name}/modify/{$data_w_read[0]['read_id']}' method='post' class = 'C_float_left'>";
...
function w_delete_form ($data_w_read, $check_loggin, $function_name) {
	...
	echo "<form action = '/minfo/{$function_name}/deletewrite/{$data_w_read[0]['read_id']}' method='post' onsubmit='return confirm_delete();' class = 'C_float_left'>";
	echo 	"<input type='hidden' name='".$data_w_read[0]['token_name']."' value='".$data_w_read[0]['hash']."' />";
	echo "<input name = 'id_check' type = 'password' value = '{$data_w_read[0]['id_check']}' class = 'C_style_input_passwd_hide'></input>";
	echo "<input name = 'read_id' type = 'password' value = '{$data_w_read[0]['read_id']}' class = 'C_style_input_passwd_hide'></input>";
...
//////////////////////////초기 댓글 구성//////////////////////////////		
		...
		//댓글 삭제를 위한 form 전송
			if($check_loggin ) {
				//로그인 되어 있으면 패스워드 검사 필요없음
				echo "<form style='display:inline-block; ' action = '/minfo/{$function_name}/deletecomment/".$entry['comment_id']."' method='post' onclick = 'delete_comment_JS(this)' onsubmit = 'return false;'>";
			} else {
				//로그인 안되어 있으면 검사 진행
				echo "<form style='display:inline-block; ' action = '/minfo/{$function_name}/deletecomment/".$entry['comment_id']."' method='post'  onclick = 'return insert_anony_passwd(this);'  onsubmit = 'return false;' >";
			}
...
<?php
					echo "<form name = 'comment_form' action = '/minfo/{$function_name}/uploadcomment' method='post'>";

					//write_page 랑 동일하게 설정할것
					if(!$check_loggin) {
						echo " <div id = 'brdRd_Dstl_CmInfo'> 
						닉네임 <input type = 'text' name = 'anony_nickname' class = 'C_style_input_nickname' placeholder = '한글기준 6자이하 '>
						비밀번호 <input type = 'password' name = 'anony_passwd' class = 'C_style_input_passwd_show' placeholder = '띄어쓰기 금지'> 
						</div>
						";
					}

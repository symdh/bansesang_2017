...
class Board_model extends CI_model { 
	...
	function __construct(){ 
		parent::__construct(); 
        $this->db_board  = $this->load->database('board', TRUE);
	...
	public function gets($setUp_pageNum) {  
      ...
      if(!is_numeric($setUp_pageNum) ) return 0;
		...
		$start_index = (int)$start_index;
				//외부값: start_index
		$result = $this->db_board->query("SELECT * FROM {$this->function_name}_writed ORDER BY read_id DESC LIMIT {$start_index}, {$setUp_pageNum}" )->result_array();

		$this->load->model('Norm_sys');
		foreach ($result as $key => $entry) {
			if($result[$key]['report_result'] == 2) $result[$key]['title'] = "[신고누적되어 Hide처리된 글입니다.]";
	...
	public function gets_info ($setUp_pageNum, $setUp_groupNum) { 
		//$setUp_pageNum = 위 gets 와 동일하게
		//$setUp_groupNum = 한 group에 몇개를 나타낼지

				//외부값: 없음
		$result = $this->db_board->query("SELECT * FROM writed_info WHERE function_name = '{$this->function_name}' ")->result_array();
		
		//일정수 이상 넘어가면 글 정보 갱신
		if($result[0]['load_count']> 100) { //최적화시 유동적으로 변경
					//외부값: 없음
			$this->db_board->query("UPDATE writed_info set load_count = 1 WHERE function_name = '{$this->function_name}' ");
			$writed_info = $this->db_board->query("SELECT count(*) as writed_count FROM {$this->function_name}_writed")->result_array();
			...
			$this->db_board->query("UPDATE writed_info set writed_count = {$writed_info[0]['writed_count']}, count_page ={$count_page}, count_group = {$count_group} WHERE function_name = '{$this->function_name}' ");
			$result = $this->db_board->query("SELECT * FROM writed_info WHERE function_name = '{$this->function_name}' ")->result_array();   //갱신된 정보를 다시 불러옴

		} else {
					//외부값: 없음
			$this->db_board->query("UPDATE writed_info set load_count = load_count + 1 WHERE function_name = '{$this->function_name}'"); 
		}
	...
	public function get_page($num) {
		...
		$num = (int)$num;
				...
				//외부값: num 
		$result = $this->db_board->query("SELECT count(*) as count FROM {$this->function_name}_writed WHERE read_id > {$num}")->result_array();
		return $result;
	}

	//집어넣은 read_id 값 반환
	private function get_insert_id () {
		$result =  $this->db_board->query("SELECT LAST_INSERT_ID()")->result_array();
	...
	public function upload_write() {
		...
		if($title === 0) {sendback_site('limit_title', $this->function_name, 'write'); return 0;}
		if($content === 0) {sendback_site('reject_access', $this->function_name, 'write'); return 0;}
		...
		if( $this->function_name == 'photo' && count($_POST["attach_image"]) == 0 ) { sendback_site('need_image', $this->function_name); return 0; } //사진 게시판: 이미지가 없을경우 리턴
		...
		if($attach_image === 0) { sendback_site('reject_access', $this->function_name, 'list'); return 0;}
		...
		$ip_address = $this->db->escape($_SERVER['REMOTE_ADDR']);
				...
				//외부값: ip_address, writed_member, passwd,  $_POST, attach_image
		$this->db_board->query("INSERT into {$this->function_name}_writed (`ip_address`,`id_check`,`passwd`,`writed_member`,`user_id`,`title`,`content`,`attach_image`) values( {$ip_address},'{$id_check}',{$passwd} ,{$writed_member} ,{$user_id} ,{$title} ,{$content} ,{$attach_image})");
		$read_id =  $this->get_insert_id();
		redirect('/board/'.$this->function_name.'/read/'.$read_id);
	}
	...
	public function download_write($num, $check_modify) { 
		...
		$num = (int)$num;
				...
				//외부값: num
		$data_w =  $this->db_board->query("SELECT * FROM {$this->function_name}_writed AS A LEFT JOIN member.user_sub_info AS B ON A.user_id = B.identification_id WHERE A.read_id = '{$num}' ")->result_array();
		if(!isset($data_w[0]['read_id'])) { sendback_site('none', $this->function_name, 'list'); return 0; } //결과 값이 없으면 거부

		//수정의 경우 데이터 검정을 함
		if ($check_modify == 1) {	
			//신고 누적상태시 거부
			if ($data_w[0]['report_result'] == 2) {
				sendback_site('exceed_report', $this->function_name, 'read',$num);
				return 0;
			}
			...
			if(isset($data_w[0]['solve_q'])) {
				//채택이 안됬고 댓글이 존재할때 
				if( $data_w[0]['solve_q'] == 0 && $data_w[0]['count_comment'] > 0) {
					//댓글이 있을경우 수정 전혀 불가
					sendback_site('reject_modify_1', $this->function_name, 'read', $num);
					return 0;
					
				} else if ( $data_w[0]['solve_q'] == 1 ) {
					//채택됬을경우 수정 전혀 불가
					sendback_site('reject_solved', $this->function_name, 'read',$num);
					return 0;
				}
			} 
			...
			if(!$this->Vui_sys->trans_pwd()  || !$this->Vui_sys->verify_board_user()) { sendback_site('wrong_passwd', $this->function_name, 'read', $num); return 0; } 
		}
		...
		if ($check_modify == 0) {
			//조회수 늘리기
					//외부값: num
			$this->db_board->query("UPDATE {$this->function_name}_writed set click_num = click_num + 1 where read_id = '{$num}' ");
	...
	public function modify_write() {
		...
		if($title === 0) { sendback_site('limit_title', $this->function_name, 'read', $read_id); return 0;}
		if($content === 0) {sendback_site('reject_access', $this->function_name, 'read', $read_id); return 0;}
		...
		if( $this->function_name == 'photo' && count($_POST["attach_image"]) == 0 ) { sendback_site('need_image', $this->function_name); return 0; } //사진 게시판: 이미지가 없을경우 리턴
				 ...
				 //외부값: $read_id
		$data_w = $this->db_board->query("SELECT * FROM {$this->function_name}_writed WHERE read_id = {$read_id} ")->result_array();
		if(!isset($data_w[0]['read_id'])) { sendback_site('none', $this->function_name, 'list'); return 0; } //결과 값이 없으면 거부
		if ($_POST['id_check'] !=$data_w[0]['id_check']){ sendback_site('reject_access', $this->function_name, 'read', $read_id); return 0; } //id 조작 여부를 확인

		//신고 누적상태시 거부
		if ($data_w[0]['report_result'] == 2) {
			sendback_site('exceed_report', $this->function_name, 'read', $read_id);
			return 0;
		}

		//개발노트 12-3,4 확인
		//질답 관련 글인지 확인 (검증이 한번 더 필요해서 넣었음)
		if(isset($data_w[0]['solve_q'])) {
			//채택이 안됬고 댓글이 존재할때 
			if( $data_w[0]['solve_q'] == 0 && $data_w[0]['count_comment'] > 0) {
				//댓글이 있을경우 수정 전혀 불가
				sendback_site('reject_modify_1', $this->function_name, 'read', $read_id);
				return 0;

			} else if ( $data_w[0]['solve_q'] == 1 ) {
				//채택됬을경우 수정 전혀 불가
				sendback_site('reject_solved', $this->function_name, 'read', $read_id);
				return 0;
			}
		} 
		...
		$this->Vui_sys->saved_id = $data_w[0]['user_id'];
		$this->Vui_sys->saved_pwd = $data_w[0]['passwd'];	
		if(!$this->Vui_sys->trans_pwd()  || !$this->Vui_sys->verify_board_user()) { sendback_site('wrong_passwd', $this->function_name, 'read', $read_id); return 0; } //유저 검증

		$saved_image = explode("%///%", $data_w[0]['attach_image']); //기존에 저장된 이미지
		...
		if($attach_image === 0) {
			sendback_site('reject_access', $this->function_name, 'read', $read_id);
			return 0;
		}
		...
		$modify_info = $this->db->escape($modify_info);
				...
				//외부값: modify_info외 다수
		$this->db_board->query("UPDATE {$this->function_name}_writed set modify_info = concat(ifnull(modify_info,''), {$modify_info}), title= {$title}, content = {$content}, attach_image = {$attach_image} where read_id= {$read_id}");

		//수정완료
		sendback_site('result_modify', $this->function_name, 'read', $read_id);
	}
	...
	public function delete_write() {
		...
		if($read_id === 0)  { sendback_site('reject_access', '', '', 0,'main'); return 0; } 
				//외부값: $read_id
		$data_w = $this->db_board->query("SELECT * FROM {$this->function_name}_writed WHERE read_id = {$read_id} ")->result_array();
		if(!isset($data_w[0])) { sendback_site('none', $this->function_name, 'list'); return 0; } //이미 삭제됬을 경우

		//개발노트 12-3,4 확인
		//질답 관련 글인지 확인
		if(isset($data_w[0]['solve_q'])) {
			//채택이 안됬고 댓글이 존재할때 (회원의 경우 삭제가능)
			if( $data_w[0]['solve_q'] == 0 && $data_w[0]['count_comment'] > 0) {
				if($data_w[0]['user_id'] == -1 ) {
					//익명의 경우
					sendback_site('reject_delete_1', $this->function_name, 'read', $read_id);
					return 0;
				}
			} else if ( $data_w[0]['solve_q'] == 1 ) {
				//채택됬을경우 삭제 전혀 불가
				sendback_site('reject_solved_1', $this->function_name, 'read', $read_id);
				return 0;
			}
		} 
		...
		if ($_POST['id_check']==$data_w[0]['id_check']){
			...
			if($result === 0) { sendback_site('reject_access', $this->function_name, 'list'); return 0; }
			
					//외부값: read_id
			$this->db_board->query("UPDATE {$this->function_name}_comment set garbage_check ='1' WHERE read_id = {$read_id} "); //댓글 쓰레기값 처리
			$this->db_board->query("DELETE from {$this->function_name}_writed WHERE read_id = {$read_id} ");  //글 삭제
			...
			sendback_site('result_delete', $this->function_name, 'list');
			return 0;
		} else {
	...
	public function lately_write () {
		...
		$_POST['lately_read_id'] = (int)$_POST['lately_read_id'];
				//외부값: _POST['lately_read_id']
		$data_w =  $this->db_board->query("SELECT * FROM {$this->function_name}_writed WHERE read_id > '{$_POST['lately_read_id']}' ")->result_array();
		...
		foreach ($data_w as $key => $entry) {
			...
			if( $this->function_name == 'photo') {
				...
				//이미지를 구한다음에 넘김 (이미지가 없는 경우는 생각하지 않음)
		          ...
		          $saved_image[0] = str_iqreplace(".", "_min.", $saved_image[0]) ;
	...
	//qaa 게시판의 채택정보를 내봄
	public function qaa_solve_info () {
		if(!is_numeric($_POST['global_read_id'])) return 0;
		$_POST['global_read_id'] = (int)$_POST['global_read_id'];
				//외부값: _POST['global_read_id']
		$data_w =  $this->db_board->query("SELECT solve_q FROM {$this->function_name}_writed WHERE read_id = '{$_POST['global_read_id']}' ")->result_array();
	...
	public function select_answer () {
		...
		if($read_id === 0)  { sendback_site('reject_access', '', '', 0,'main'); return 0; } 
		if($comment_id === 0) { sendback_site('reject_access', $this->function_name, 'read', $read_id); return 0; }

				//외부값: read_id
		$data_w = $this->db_board->query("SELECT * FROM {$this->function_name}_writed WHERE read_id = {$read_id} ")->result_array();
		if(!isset($data_w[0]['read_id'])) { sendback_site('reject_access', $this->function_name, 'read', $read_id); return 0; } //결과 없으면 리턴

		//이미 채택되어 있다면 리턴
		if($data_w[0]['solve_q']) {
			sendback_site('reject_choose', $this->function_name, 'read', $read_id);
			return 0;
		}
		...
		$user_id =$this->db->escape($this->Vui_sys->call_user_id());
				...
				//외부값: comment_id
		$data_c =  $this->db_board->query("SELECT * FROM {$this->function_name}_comment WHERE comment_id = {$comment_id} ")->result_array();

		//결과가 없거나 다른글의 댓글이라면 리턴
		if(!isset($data_c[0]['selected_a'])) {
			sendback_site('reject_access', $this->function_name, 'read', $read_id);
			return 0;
		} else if ( $data_c[0]['read_id'] != $read_id) {
			sendback_site('reject_access', $this->function_name, 'read', $read_id);
			return 0;
		}

		//익명 댓글일시 리턴
		if( $data_c[0]['user_id'] == -1 ){
			sendback_site('reject_choose_anony', $this->function_name, 'read', $read_id);
			return 0;
		}

		//본인 댓글일때 리턴
		if($user_id ==$data_c[0]['user_id'] ) {
			sendback_site('reject_choose_myself', $this->function_name, 'read', $read_id);
			return 0;
		} 

		//댓글 업데이트
				//외부값: comment_id
		$this->db_board->query("UPDATE {$this->function_name}_comment set selected_a =  1 where comment_id = {$comment_id}  ");
		$data_c[0]['user_id'] = $this->db->escape($data_c[0]['user_id']);
		//글 업데이트
				//외부값: comment_id, read_id
		$this->db_board->query("UPDATE {$this->function_name}_writed set solve_q =  1, select_member = {$data_c[0]['user_id']} where read_id = '{$read_id}' ");

		//완료
		sendback_site('result_choose', $this->function_name, 'read', $read_id);
		return 0;
...
//////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public function upload_comment() {
		...
		// g1을 불러옴
				//외부값:read_id
		$this->db_board->query("UPDATE {$this->function_name}_writed set max_group_1_id = @var1 := max_group_1_id +1 WHERE read_id = {$read_id} ");
		$result = $this->db_board->query("SELECT @var1 ")->result_array();
		...
		$ip_address = $this->db->escape($_SERVER['REMOTE_ADDR']);
			...
			//외부값: 다수
		$this->db_board->query("INSERT into {$this->function_name}_comment (`ip_address`,`read_id`,`group_1_id`,`id_check`,`passwd`,`writed_member`,`user_id`,`content`,`super_user`) values({$ip_address}, {$read_id}, '".$result[0]['max_group_1_id']."', '".$id_check ."', {$passwd}, {$writed_member}, {$user_id}, {$title}, {$super_user})");
		
		//글의 댓글 갯수 정보를 업데이트함
				//외부값: read_id
		$this->db_board->query("UPDATE {$this->function_name}_writed set count_comment = count_comment + 1 where read_id = {$read_id}  ");
	...
	public function upload_answer_comment () {
		...
		//g1 으로 글 삭제 됬는지 체크
				//외부값: read_id
		$check_delete = $this->db_board->query("SELECT * FROM {$this->function_name}_writed WHERE read_id = {$read_id} ")->result_array();
		...
		$result = $this->db_board->query("SELECT max(DISTINCT group_2_id) as max_g2 FROM {$this->function_name}_comment WHERE read_id = {$read_id} AND group_1_id = {$group_1_id} ")->result_array();
		...
		$ip_address = $this->db->escape($_SERVER['REMOTE_ADDR']);
				...
				//외부값: 다수
		$this->db_board->query("INSERT into {$this->function_name}_comment (`ip_address`,`read_id`,`depth`,`group_1_id`,`group_2_id`,`id_check`,`passwd`,`writed_member`,`user_id`,`send_member`,`content`,`super_user`) values( {$ip_address}, {$read_id},'".$depth_comment."', {$group_1_id}, '".$result[0]['max_g2']."', '".$id_check ."', {$passwd}, {$writed_member}, {$user_id}, {$send_member}, {$title}, {$super_user})");
	...
	public function download_comment($num) {
		...
		$num = (int)$num;
				//외부값: num
		$data_c =  $this->db_board->query("SELECT * FROM {$this->function_name}_comment AS A LEFT JOIN member.user_sub_info AS B ON A.user_id = B.identification_id WHERE A.read_id = '{$num}' ORDER BY group_1_id ASC, comment_id ASC ")->result_array();
	...
	// 	$result = $this->db_board->query("SELECT * FROM {$this->function_name}_comment WHERE comment_id = '{$_POST['comment_id']}' ")->result_array();
	...
	// 	$this->db_board->query("UPDATE {$this->function_name}_comment set content ='{$_POST['content']}' WHERE comment_id = '{$_POST['comment_id']}' ");
	...
	public function delete_comment ($num ) {
		...
		if(!$this->Vui_sys->trans_pwd()  || !$this->Vui_sys->verify_board_user()) { echo "패스워드가 틀립니다."; return 0; } //유저 검증
				...
				//외부값: comment_id
		$this->db_board->query("DELETE from {$this->function_name}_comment WHERE comment_id = {$comment_id} ");
		//글의 댓글 갯수 정보를 업데이트함
				//외부값: 없음
		$this->db_board->query("UPDATE {$this->function_name}_writed set count_comment = count_comment - 1 where read_id = '{$data_c['0']['read_id']}'  ");
	}  
	...
	public function lately_comment () {
		...
		$_POST['global_read_id'] = (int)$_POST['global_read_id'];
				//외부값: _POST['global_read_id'] 
		$data_c =  $this->db_board->query("SELECT * FROM {$this->function_name}_comment AS A LEFT JOIN member.user_sub_info AS B ON A.user_id = B.identification_id WHERE A.read_id = '{$_POST['global_read_id']}' ORDER BY group_1_id ASC, comment_id ASC ")->result_array();
